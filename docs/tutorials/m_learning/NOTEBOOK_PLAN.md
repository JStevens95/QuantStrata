# Plan: m_learning Lifecycle Tutorial Notebook
...
|--------|--------|--------|
| 0 | **Overview & pipeline map** | High-level lifecycle diagram; table of pipelines and their inputs/outputs; where each component lives in `src/m_learning` |
...
- **Markdown:**
  ...
  - **Tree:** Short code or text showing `src/m_learning` layout (core, data, pipeline, training, evaluation, inference, models).

**Deliverables:** One markdown cell with table + diagram; one small code cell that prints the package layout (e.g. `import m_learning; print(dir(...))` or a static tree).
...
- **Filename:** `ml_lifecycle_tutorial.ipynb` (or `m_learning_lifecycle.ipynb`).
- **Location:** `docs/tutorials/m_learning/`.
