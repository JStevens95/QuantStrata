#!/usr/bin/env python3
"""
Graph-Aligned Toy Regression: Hybrid GNN-RNN Model
===================================================
...
from src.rade_ml.data.hybrid_gnn_rnn.build import encode_trade_attributes, build_trade_graph
from src.rade_ml.data.hybrid_gnn_rnn.config import GraphBuilderConfig, AttributeEncoderConfig
...
logger = logging.getLogger("example.hybrid_gnn_rnn.graph_aligned_toy")
...
# ======================================================================
#
# Target PnL = weighted mean of graph-neighbors' elementary PnL.
# Uses the SAME graph the model uses, so the relationship is learnable.
# ======================================================================
...
def make_graph_aligned_toy_data(
    ...
    Matches the model's design: targets aggregate from neighbors in attribute space.
    """
    ...
    # Encode attributes (same as pipeline)
    ...
    combined = encoder_results["combined_encoded"]

    # Build graph (same as pipeline)
    ...
    adj = builder.adjacency_dense  # [n_trades, n_trades], row-normalized

    # Target indices in graph: [n_elementary, n_elementary + n_target)
    n_elem, n_tgt = n_elementary, n_target
    adj_tgt_elem = adj[n_elem : n_elem + n_tgt, :n_elem]  # [n_tgt, n_elem]

    # Elementary PnL
    elem_pnl_arr = rng.randn(n_scenarios, n_elementary).astype(np.float32) * 0.01

    # Target = graph-weighted mean of elementary neighbors
    # tgt_pnl[s, i] = sum_j adj_tgt_elem[i,j] * elem_pnl[s,j]
...
# ======================================================================
# 1.  Configuration (toy-friendly: lower dropout)
# ======================================================================
...
def build_configs(workdir: Path, job: dict) -> "PipelineConfig":
    ...
    # Toy-friendly model config: lower dropout so model can learn
    model_config = {
        "general": {},
    ...
    config = PipelineConfig(
        ...
        metadata={"job": job, "run_name": "hybrid_gnn_rnn_graph_aligned_toy", "generate_training_report": False},
    )
    return config
...
def run_step_by_step(config: "PipelineConfig"):
    ...
    # Baseline: graph-weighted mean (we'd need to recompute from adj; use simple mean as proxy for comparison)
    ...
    if r2_test is not None:
        if r2_test > 0.5:
            print("  → PASS: Model learned graph-aligned aggregation (confidence for real exotics)")
        elif r2_train > 0.5:
    ...
    registry.register(
        ...
        description="Hybrid GNN-RNN graph-aligned toy (target=graph-weighted mean)",
    )
    print("  Registered.")
...
def main():
    workdir = Path(tempfile.mkdtemp(prefix="rade_ml_graph_aligned_toy_"))
