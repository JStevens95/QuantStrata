#!/usr/bin/env python3
"""
Toy Regression Example: Hybrid GNN-RNN Model (PyTorch)
======================================================
...
If the model is working correctly, it should achieve:
  ...
  - R^2 close to 1.0 on the test set
  - RMSE << 1 (in standardized space)

Run: ``python examples/rade_ml_pt/hybrid_gnn_rnn/02_train_hybrid_gnn_rnn_toy_regression.py``
"""
...
logger = logging.getLogger("example.hybrid_gnn_rnn_pt.toy_regression")
...
def make_toy_regression_data(
    ...
    tgt_pnl_arr = tgt_pnl_arr + rng.randn(n_scenarios, n_target).astype(np.float32) * noise_std
...
def build_configs(workdir: Path, job: dict) -> "PipelineConfig":
    ...
    config = PipelineConfig(
        ...
        metadata={
            "job": job,
            "run_name": "hybrid_gnn_rnn_pt_toy_regression",
...
def run_step_by_step(config: "PipelineConfig"):
    ...
    entry = registry.register(
        ...
        description="Hybrid GNN-RNN PT toy regression (target=mean elementary)",
    )
...
def main():
    workdir = Path(tempfile.mkdtemp(prefix="rade_ml_pt_toy_regression_"))
