...
This module contains TF-native learning rate schedule classes that are
used by ``LRScheduleConfig.build()`` in ``core/config.py``.  Keeping them
here avoids forcing a TensorFlow import at ``config.py`` import time,
which means the pure-Python configuration dataclasses can be loaded,
serialised, and transmitted without TensorFlow installed.
...
class WarmupCosineSchedule(tf.keras.optimizers.schedules.LearningRateSchedule):
    ...
    def __call__(self, step):
        ...
        # Linear warmup: 0 → initial_lr over warmup_steps
        warmup_lr = self.initial_lr * (step / tf.maximum(warmup_steps, 1.0))

        # Cosine decay: initial_lr → min_lr over decay_steps
        progress = (step - warmup_steps) / tf.maximum(decay_steps, 1.0)
        progress = tf.clip_by_value(progress, 0.0, 1.0)
        cosine_lr = (
            self.min_lr
            + 0.5 * (self.initial_lr - self.min_lr) * (1.0 + tf.cos(3.14159 * progress))
        )
