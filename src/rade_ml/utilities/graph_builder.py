"""
Trade graph builder for Hybrid GNN-RNN model.
...
class TradeGraphBuilder:
    ...
    # Maps encoded trade attribute keys → the corresponding alpha weight attribute
    ...
    def adjacency_dense(self) -> NDArray | None:
        ...
        save). The GNN forward pass should use the tf.SparseTensor returned
    ...
    def build_graph(
    ...
    ) -> dict[str, tf.SparseTensor | NDArray]:
        """
        ...
        :return: dict with adjacency_matrix (SparseTensor), sparse_indices, values, shape, is_target.
        """
    ...
    def build_graph_projection(
        ...
        - 0:      no edges from original → new (original embeddings are unchanged).
    ...
    def _build_knn_adjacency(
        ...
        Algorithm:
          ...
          3. Convert distances → similarity via Gaussian RBF kernel.
          ...
          7. Export as (indices, values, dense) for tf.SparseTensor and visualisation.
        ...
        :return: (indices [nnz, 2], values [nnz], dense [n, n]).
        """
        n = features.shape[0]

        # --- Step 1-2: k-NN search ---
        # include_quota uses the bipartite-aware search that guarantees minimum
        # elementary and target neighbours for each target trade.
        if include_quota:
        ...
        # Converts raw distances to similarity weights in [0, 1] with per-row
        # adaptive bandwidth. Nearby trades → weight ≈ 1, distant → weight → 0.
        w = self._rbf_weights(dists)

        # --- Step 4: Assemble sparse COO ---
        # Each node i contributes k edges: (i, neighbour_j) with weight w_ij.
        rows = np.repeat(np.arange(n), k_actual)
        cols = inds.ravel()
        data = w.ravel()

        # --- Step 5: Self-loops ---
        # Weight 1.0 ensures the node's own features contribute to the neighbourhood
        # aggregation. After row-norm, self-weight = 1 / (1 + sum(rbf_weights)).
        if add_self_loops:
            diag = np.arange(n)
        ...
        # D^{-1}A: each row sums to 1. The matmul A @ X now computes a weighted
        # mean of neighbour features, which is exactly what GraphSAGE expects.
    ...
    def _extend_adjacency(
        ...
        freeze_original=True  : original rows preserved verbatim, new rows row-normalised.
        freeze_original=False : symmetric edges added, full matrix re-normalised.
        ...
        # The NearestNeighbors index is built on all features (original + new).
        # Each new trade queries its k nearest neighbours, which may be original
        # or other new trades depending on include_new_new.
        ...
        dists, inds = dists[:, 1:], inds[:, 1:]  # drop self
        k_actual = inds.shape[1]

        # Same RBF kernel as training — consistency is critical for the GNN
        # to generalise learned weight matrices to unseen trades.
        w = self._rbf_weights(dists)

        # --- Vectorised edge assembly (no Python loops) ---
        # Forward edges: new_trade → its k neighbours.
        rows_fwd = np.repeat(new_idx, k_actual)
        cols_fwd = inds.ravel().astype(np.int64)
        data_fwd = w.ravel()

        # Drop new→new edges when not allowed (keep only edges to original trades).
        if not include_new_new:
            keep = cols_fwd < n_orig
            rows_fwd, cols_fwd, data_fwd = rows_fwd[keep], cols_fwd[keep], data_fwd[keep]

        # Reverse edges: neighbour → new_trade. Controls whether original nodes
        # "see" new trades in their aggregation.
        # - freeze_original=True:  only add reverse for new↔new edges (not old→new),
        #   so original node embeddings are unchanged at inference.
        # - freeze_original=False: add all reverse edges, then re-normalise everything.
        ...
        # Self-loops for new nodes: weight 1.0 (same convention as training) so the
        # GNN's dense_self pathway has matching input from the adjacency self-loop.
        rows_self = new_idx
        cols_self = new_idx
        data_self = np.ones(n_new, dtype=np.float32)

        # Concatenate all edge types into a single COO array.
        ...
        # Top-left block = original trained adjacency (already row-normalised).
        # New edges added via COO to avoid modifying the original block in-place.
        ...
        # --- Row normalisation ---
        # freeze_original: normalise the entire matrix, then overwrite original rows
        # with their pre-normalisation values. This ensures new rows sum to 1 while
        # original rows remain bit-for-bit identical to training (avoids float drift).
        if freeze_original:
    ...
    def _generate_neighbours_quota(
        ...
        The trade universe is bipartite: elementary trades (hedging instruments) and
        target trades (portfolio positions to predict). For the GNN to learn the
        relationship between targets and their hedges, each target trade must have
        sufficient neighbours from BOTH partitions:

          - p_min_elementary: guaranteed elementary neighbours (hedging context).
          - q_min_target:     guaranteed target neighbours (peer structure).
          - remaining slots:  filled by nearest overall (regardless of type).

        Elementary nodes have no quota constraints — they simply take their k nearest.

        Uses sklearn NearestNeighbors for O(n log n) candidate search, then applies
        quota-based reranking per target node.
        ...
        # k = actual number of neighbours per node (excluding self).
        ...
        # Fetch a generous candidate pool (3x k) so that after partitioning into
        # elementary/target there are enough candidates to fill both quotas.
        candidate_k = min(
        ...
        # Pre-computed boolean array for O(1) partition lookup by index.
        is_elem = ~is_target
        ...
        for node in range(n):
            ...
            if not is_target[node]:
                # Elementary nodes: no quota — just take the k nearest overall.
                sel_i = cand_i[:k]
                sel_d = cand_d[:k]
            else:
                # Target nodes: partition candidates into elementary vs target
                # using boolean fancy indexing (vectorised, no set lookups).
                ...
                # Determine how many from each partition to take, respecting quotas
                # but not exceeding k total or the available candidates.
                need_e = min(self.p_min_elementary, len(e_i), k)
                need_t = min(self.q_min_target, len(t_i), max(0, k - need_e))

                # Pick the nearest from each partition (already distance-sorted by sklearn).
                parts_i, parts_d = [], []
                if need_e > 0:
                ...
                # Fill remaining slots with the nearest unused candidates
                # regardless of type — this maximises local information.
                ...
                # Sort by (distance, index) for deterministic output regardless
                # of the order in which quota picks and fill picks were gathered.
                if sel_i.size > 1:
                    order = np.lexsort((sel_i, sel_d))
                    sel_i, sel_d = sel_i[order], sel_d[order]

            # Pad with the farthest neighbour if fewer than k candidates exist
            # (only possible in very small graphs). The duplicate index will be
            # de-duplicated when converted to COO (values summed), but this is
            # a rare edge case that doesn't affect production workloads.
            if 0 < len(sel_i) < k:
                pad = k - len(sel_i)
    ...
    def _weighted_features(self, encoded_trades: dict[str, NDArray]) -> NDArray:
        ...
        in this space equals sum_d( alpha_d * (x_id - x_jd)^2 ). This is a diagonal
        Mahalanobis distance giving explicit control over which trade attributes
        matter most for graph connectivity.

        Additional scaling for high-dimensional features:
        - One-hot embeddings: divided by sqrt(dim) so a 20-class product type doesn't
          contribute 20x more to distance than a single scalar feature.
        - Multi-label (risk factors): L2-normalised per row so trades with many risk
          factors aren't artificially more distant from trades with few.
        """
        parts: list[NDArray] = []
        ...
        # Scaled by 1/sqrt(dim) via _scale_embedding() to normalise their total
        # contribution to distance regardless of the number of categories.
        for key, alpha_attr in self._EMBEDDING_ALPHA_MAP.items():
            alpha = getattr(self, alpha_attr)
            if key in encoded_trades and alpha > 0.0:
                parts.append(np.sqrt(alpha) * self._scale_embedding(encoded_trades[key]))

        # Multi-label: underlying_risk_factors. L2-normalised per trade so the
        # distance reflects which risk factors are present, not how many.
    ...
    def _rbf_weights(dists: NDArray) -> NDArray:
        ...
        Dense clusters get tight sigma (sharp similarity), sparse regions get wider
        sigma (more permissive). Floor of 1e-9 prevents division by zero for
        duplicate-feature nodes.
        """
    ...
    def _row_normalise(adj) -> csr_matrix:
        ...
        This is the normalisation required by GraphSAGE's mean aggregator:
        (A_hat @ X)_i = weighted mean of x_j over j in N(i) ∪ {i}.

        Zero-degree rows (no edges) get inv=0 to avoid NaN.
        """
    ...
    def _scale_embedding(emb: NDArray) -> NDArray:
        ...
        - Ensures 2D shape (handles edge case of single-category encodings).
        - Divides by sqrt(dim): a 20-class one-hot has 20 columns that would
          contribute 20x to distance vs a scalar feature. Dividing by sqrt(20)
          normalises the total squared contribution to ~1 per alpha unit.
        """
        if emb.ndim == 1:
            emb = emb.reshape(-1, 1)
    ...
    def _to_sparse_output(adj: Any) -> tuple[NDArray, NDArray, csr_matrix]:
        ...
        Convert scipy sparse matrix to sparse arrays + CSR for downstream use.
          ...
          - indices [nnz, 2]: row/col pairs for tf.SparseTensor.
          - values  [nnz]:    normalised edge weights.
          - csr:              compressed sparse row for efficient storage and element access.

        Dense conversion (.toarray()) is deferred to the adjacency_dense property
        so the GNN forward pass never pays for a full N×N materialisation.
        """
        coo = adj.tocoo()
    ...
    def _pack_result(
        ...
        ``adjacency_matrix`` is now the tf.SparseTensor used directly by the GNN
        ...
        tensor = tf.SparseTensor(indices=indices, values=values, dense_shape=shape)
        return {
            "adjacency_matrix": tensor, "sparse_tensor": tensor, "sparse_indices": indices,
            "sparse_values": values, "sparse_shape": shape, "is_target": is_target,
        }
    ...
    def _adjust_k_and_quotas(self, num_elementary: int, num_target: int) -> None:
        ...
        Called before graph construction to handle small trade universes where
        the configured k or quotas exceed the actual number of available trades.
        """
        if self.k > num_elementary:
            self.k = max(1, num_elementary // 2) if num_elementary > 1 else num_elementary
            logger.warning(f"Adjusted k to {self.k} (only {num_elementary} elementary trades).")
    ...
    def _validate_constraints(self, p: int, q: int, k: int) -> None:
        ...
        If the user configures p_min + q_min > k, there aren't enough neighbour
        slots to satisfy both quotas. We clamp rather than raise so the builder
        is usable with reasonable defaults even on small graphs.
        """
        if p + q > k:
