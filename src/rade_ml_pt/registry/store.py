"""
Local filesystem model registry for PyTorch.
...
Storage layout::
    ...
    root_dir/
      {version}/
        model.pt             # Full pickled model (torch.save)
...
class ModelRegistry:
    """
    Local filesystem registry for trained PyTorch models.
    ...
    def register(
        ...
        The model is serialised via ``torch.save(model, path)`` which pickles
        the full module (architecture + weights).  This mirrors the behaviour
        of ``tf.keras.Model.save()`` in the TensorFlow counterpart.
        ...
        # save the full model (architecture + weights) via pickle
    ...
    def load(
        ...
        Returns
        -------
        tuple of (nn.Module, RegistryEntry)
        """
        ...
        model = torch.load(
            str(version_dir / _MODEL_FILENAME),
            map_location=loc,
            weights_only=False,
        )
        model.eval()
