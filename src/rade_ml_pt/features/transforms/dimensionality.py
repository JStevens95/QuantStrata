...
from src.rade_ml_pt.validation.exceptions import UndefinedReductionType, UndefinedComputationMethod

from src.rade_ml_pt.data.hybrid_gnn_rnn.config import DimensionalityConfig, BasisSelectionConfig
...
def run_dimensionality_reduction(
    ...
    if config.reduction_mode.lower() == "none":
        return list(pnl_df.columns)
...
def select_minimal_basis(pnl_df: pd.DataFrame, k: int, weight_tail: float = 1.0) -> List[str]:
    ...
    selected: List[str] = [pnl_df.columns[i] for i in pivots[:k]]
    return selected
